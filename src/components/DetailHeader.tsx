...
const DetailHeader = ({ onLanguageChange, selectedLanguage }: DetailHeaderProps) => {
    ...
    <div className="flex items-center justify-between p-4 bg-white border-b border-gray-200">
      ...
      <div className="flex items-center gap-3">
        <h1 className="text-xl font-semibold text-gray-900">
          {getTranslation('hello', selectedLanguage)} {getStaffName()}!
        </h1>
      </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="flex items-center gap-2 hover:bg-gray-50">
              <Avatar className="h-12 w-12 border-2" style={{ borderColor: 'rgba(186, 43, 31, 1)' }}>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => handleMenuItemClick('/manage-customer')}>
              ...
              {getTranslation('auditees', selectedLanguage)}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleMenuItemClick('/knowledge-data')}>
              <Settings className="h-4 w-4 mr-2" />
              {getTranslation('evidenceTuning', selectedLanguage)}
            </DropdownMenuItem>
          ...
          <DropdownMenuContent align="end">
            {languages.map((language) => (
              <DropdownMenuItem
                ...
                className={selectedLanguage === language.code ? 'bg-gray-100' : ''}
              >
                <img 
                  src={language.flag} 
                  alt={language.name}
                ...
                />
                {language.code} - {getTranslation(language.code === 'EN' ? 'english' : 'french', selectedLanguage)}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
